...
class projectsController {
  async createProject(req, res, next) {
    try {
      ...
      const updatedProjects = await projectService.createNewProject(
        req.user.id,
        req.body.name,
        req.body.link
      );
  ...
  async changeProject(req, res, next) {
    try {
      ...
      const updatedProjects = await projectService.changeProject(
        req.user.id,
        projectId,
        name,
        link
  ...
  async deleteProject(req, res, next) {
    try {
      const updatedProjects = await projectService.deleteProject(
        req.user.id,
  ...
  async addProjectScripts(req, res, next) {
    try {
      ...
      const updatedProject = await projectService.addScripts(
        projectId,
        scripts
      );
  ...
  async changeStatus(req, res, next) {
    try {
      ...
      const updatedProject = await projectService.changeStatus(
        projectId,
        props
      );
  ...
  async setFontConfigs(req, res, next) {
    try {
      ...
      const { projectId, fontFamily, titleSize, titleWeight, textSize } =
        req.body;

      const updatedProject = await projectService.setFontConfigs(
        projectId,
        fontFamily,
        titleSize,
        titleWeight,
  ...
  async formProcessing(req, res, next) {
    try {
      ...
      const { projectId, email, secondaryEmail, letterSubject, phoneNumber } =
        req.body;

      const updatedProject = await projectService.formProcessing(
        projectId,
        email,
        secondaryEmail,
        letterSubject,
  ...
  async getAllProjects(req, res, next) {
    try {
      ...
      if (!projects)
        return next(ApiError.BadRequest('Не найден ни один проект', 'danger'));
